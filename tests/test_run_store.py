...
def test_run_store_writes_trace_and_report_as_readable_utf8(tmp_path):
    store = RunStore(tmp_path / ".pico" / "runs")
    state = TaskState.create(run_id="run_utf8", task_id="task_utf8", user_request="读取项目")
    store.start_run(state)

    store.append_trace(state, {"event": "run_started", "user_request": "读取项目"})
    state.finish_success("完成")
    store.write_task_state(state)
    store.write_report(state, {"final_answer": "完成"})

    trace_text = store.trace_path(state.run_id).read_text(encoding="utf-8")
    report_text = store.report_path(state.run_id).read_text(encoding="utf-8")
    task_text = store.task_state_path(state.run_id).read_text(encoding="utf-8")

    assert "读取项目" in trace_text
    assert "\\u8bfb" not in trace_text
    assert "完成" in report_text
    assert "完成" in task_text
