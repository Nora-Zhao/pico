...
def test_session_store_writes_readable_utf8(tmp_path):
    agent = build_agent(tmp_path, ["<final>完成。</final>"])

    assert agent.ask("读取 runtime.py") == "完成。"

    session_text = Path(agent.session_path).read_text(encoding="utf-8")
    assert "读取 runtime.py" in session_text
    assert "完成。" in session_text
    assert "\\u8bfb" not in session_text
    assert "\\u5b8c" not in session_text
...
def test_terminal_model_printer_streams_final_content_without_tags():
    stream = StringIO()
    printer = TerminalModelPrinter(stream=stream)

    for delta in ["<fi", "nal>Hel", "lo</fi", "nal>"]:
        printer.feed(delta)
    printer.finish()
...
def test_terminal_model_printer_hides_tool_call_text():
    ...
    printer.feed('<tool>{"name":"read_file","args":{"path":"README.md"}}</tool>')
    printer.finish()

    assert stream.getvalue() == ""


def test_terminal_model_printer_shows_runtime_progress_without_suppressing_final_answer():
    stream = StringIO()
    printer = TerminalModelPrinter(stream=stream)
    ...
    printer.feed_event(
        {
            ...
            "kind": "tool",
            "tool_name": "read_file",
            "tool_args": {"path": "pico/tools.py", "start": 1, "end": 80},
        }
    )
    printer.feed_event(
        {
            "event": "tool_executed",
            "name": "read_file",
            "args": {"path": "pico/tools.py", "start": 1, "end": 80},
            "tool_status": "ok",
            "duration_ms": 2,
        }
    )
...
def test_openai_chat_completions_client_posts_expected_streaming_payload():
    captured = {}
    ...
    class FakeResponse:
        ...
        lines = [
            ...
            b"\n",
            b'data: {"choices":[{"delta":{"content":"ok</final>"},"finish_reason":null}]}\n',
            b"\n",
            b'data: {"choices":[],"usage":{"prompt_tokens":10,"completion_tokens":2,"total_tokens":12,"prompt_cache_hit_tokens":3}}\n',
            b"\n",
            b"data: [DONE]\n",
            b"\n",
        ]
    ...
    assert client.last_completion_metadata["total_tokens"] == 12
    assert client.last_completion_metadata["cached_tokens"] == 3
    assert client.last_completion_metadata["cache_hit"] is True
...
def test_openai_compatible_client_extracts_text_from_event_stream():
    class FakeResponse:
        ...
        lines = [
            ...
            b"\n",
            b'data: {"type":"response.output_text.delta","delta":"<final>stream ok</final>"}\n',
            b"\n",
            b'data: {"type":"response.completed","response":{"usage":{"input_tokens":8,"output_tokens":4,"total_tokens":12}}}\n',
            b"\n",
            b"data: [DONE]\n",
            b"\n",
        ]
    ...
    assert client.last_completion_metadata["input_tokens"] == 8
...
def test_openai_compatible_client_retries_on_incomplete_read():
    calls = {"count": 0}
    ...
    with patch("urllib.request.urlopen", return_value=FakeResponse()), patch("time.sleep", return_value=None):
        result = client.complete("hello", 42)
...
def test_anthropic_compatible_client_retries_on_incomplete_read():
    calls = {"count": 0}
...
def test_build_arg_parser_accepts_deepseek2_provider(tmp_path):
    args = mini_pkg.build_arg_parser().parse_args(["--cwd", str(tmp_path), "--provider", "deepseek2"])

    assert args.provider == "deepseek2"
...
def test_build_agent_uses_deepseek2_chat_completions_endpoint(tmp_path):
    args = type(
        "Args",
        (),
        {
    ...
    with patch.dict(
        os.environ,
        {
            ...
            "PICO_DEEPSEEK2_API_KEY": "sk-project-deepseek2",
        ...
        ), patch("pico.cli.OpenAICompatibleModelClient", side_effect=AssertionError("responses client should not be used")), patch(
    ...
    assert mock_chat.call_args.kwargs["api_key"] == "sk-project-deepseek2"
    assert agent.model_client is fake_client
